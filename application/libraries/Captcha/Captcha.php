...
class Captcha {
	 ...
	 * @var string
	 */
	public static $_codeSet   = '346789ABCDEFGHJKLMNPQRTUVWXY';
	...
	public static function entry($id = '') {
		...
		// 验证码使用随机字体
		$ttfPath = dirname(__FILE__) . '/' . (self::$useZh ? 'zhttfs' : 'ttfs') . '/';
		...
		for ($i = 0; $i<self::$length; $i++) {
			if(self::$useZh) {
				...
				$code[$i] = self::$_codeSet[mt_rand(0, (strlen(self::$_codeSet)-1))];
		...
		for($i = 0; $i < 10; $i++){
			...
			for($j = 0; $j < 5; $j++) {
			    ...
			    imagestring(
			        ...
			        self::$_codeSet[mt_rand(0, (strlen(self::$_codeSet)-1))], // 杂点文本为随机的字母或数字
			        $noiseColor
			    );
			}
		}
