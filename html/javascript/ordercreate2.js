...
var PageOrdercreate2 = {
    ...
    construct: function()
    {
        ...
        if ($.getLocalCache('chooseAddress'))
        {
            ...
            PageOrdercreate2._addressid = chooseAddress.addressid;
            $('#jsid').val(chooseAddress.CERTNO == null ? '':chooseAddress.CERTNO);
            ...
            if (shopCarInfo.address.length > 0)
            {
                ...
                PageOrdercreate2._addressid = shopCarInfo.address[0].Id;
            }
            else {
    ...
    buy: function()
    {
        ...
        $('#jspay').click(function() {
            ...
            if (PageOrdercreate2._addressid == 0)
            {
            ...
            if($.getUrlData().type && $.getUrlData().type == 'discount')
            {
                $.getApi({
                    'discountId': $.getUrlData().id,
                    'address': PageOrdercreate2._addressid,
                    'buyerMsg': $('#jsmsg').val(),
                    'certNo': idcard,
                    'num': $.getUrlData().num,
                    'cuppons': shopCarInfoCoupons
                }, 'front/order/createorderbydiscountid', 'CallBack.createOrder');
            }
            else{
                $.getApi({
                    ...
                    'num': $.getUrlData().num,
                    'cuppons': shopCarInfoCoupons
                }, 'front/order/createordernyprodid', 'CallBack.createOrder');
            }
            return;
    ...
    defaultEvent: function()
    {
        $('#jschooseaddress').click(function() {
            if($.getUrlData().type == 'discount')
            {
                window.location.href = 'address.html?back=ordercreate2&id='+ $.getUrlData().id+'&num='+ $.getUrlData().num;
            }
            else{
               // window.location.href = 'address.html?back=ordercreate2';
                window.location.href = 'address.html?back=ordercreate2&id='+ $.getUrlData().id+'&num='+ $.getUrlData().num;
            }
